...
const PatrolCalendar = () => {
  ...
  const getReportsForDate = (date: Date) => {
    return reports?.filter((r) => isSameDay(new Date(r.start_time), date)) || [];
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-bold">巡逻日历</h1>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
                  ...
                  >
                    ...
                    {dayReports.length > 0 && (
                      <div className="absolute bottom-1 left-1/2 transform -translate-x-1/2 flex gap-0.5">
                        ...
                        {dayReports.length > 3 && (
                          <span className="text-xs text-muted-foreground">+{dayReports.length - 3}</span>
                        )}
                      </div>
                    )}
                  </button>
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              {selectedDate
                ? format(selectedDate, "M月d日 巡逻记录", { locale: zhCN })
                : "选择日期查看详情"}
            </CardTitle>
          </CardHeader>
          <CardContent>
                  ...
                  {selectedReports.map((report) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-medium">{report.guards?.name || "未知"}</span>
                        <Badge
                          variant={
